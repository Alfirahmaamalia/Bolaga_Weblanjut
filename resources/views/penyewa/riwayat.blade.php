@extends('layouts.app')

@section('title', 'Riwayat Booking')

@section('content')

<div class="max-w-7xl mx-auto px-4 py-6">

    <a href="{{ route('penyewa.dashboard') }}" 
        class="inline-flex items-center text-gray-700 hover:text-black mb-4">
        
        <svg xmlns="http://www.w3.org/2000/svg" 
            class="w-5 h-5 mr-1 stroke-current" 
            fill="none" viewBox="0 0 24 24">
            <path d="M15 19l-7-7 7-7" 
                stroke-width="2" 
                stroke-linecap="round" 
                stroke-linejoin="round" />
        </svg>

        <span>Kembali</span>
    </a>

    <!-- Section Title -->
    <section class="text-center mb-10">
        <h1 class="text-3xl font-bold">RIWAYAT BOOKING</h1>
        <p class="text-gray-600">Lihat daftar penyewaan lapangan yang pernah kamu lakukan</p>
    </section>


    <!-- TABLE WRAPPER (AGAR BISA SCROLL KE KANAN/KIRI) -->
    <div class="bg-white rounded-xl shadow-md p-6 overflow-x-auto">

        <table class="w-full min-w-[900px]">
            <thead>
                <tr class="bg-green-600 text-white text-left border-b">
                    <th class="p-3 font-semibold">No</th>
                    <th class="p-3 font-semibold">Nama Lapangan</th>
                    <th class="p-3 font-semibold">Tanggal Sewa</th>
                    <th class="p-3 font-semibold">Jam Sewa</th>
                    <th class="p-3 font-semibold">Status</th>
                    <th class="p-3 font-semibold">Total Harga</th>
                    <th class="p-3 font-semibold">Metode Pembayaran</th>
                    <th class="p-3 font-semibold">Bukti Pembayaran</th>
                    <th class="p-3 font-semibold">Tanggal Booking</th>
                    <th class="p-3 font-semibold">Aksi</th>
                </tr>
            </thead>

            <tbody>
                @foreach ($riwayat as $i => $row)
                    <tr class="border-b hover:bg-gray-50">
                        <td class="p-3">{{ $i + 1 }}</td>
                        <td class="p-3 font-semibold">{{ $row->lapangan->nama_lapangan }}</td>
                        <td class="p-3">{{ date('d-m-Y', strtotime($row->tanggal)) }}</td>
                        <td>
                            {{ date('H:i', strtotime($row->jam_mulai)) }}
                            -
                            {{ date('H:i', strtotime($row->jam_selesai)) }}
                        </td>

                        <td class="p-3">
                            @if($row->status == 'berhasil')
                                <span class="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs">
                                    {{ $row['status'] }}
                                </span>
                            @elseif($row->status == 'belum bayar')
                                <span class="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full text-xs">
                                    {{ $row['status'] }}
                                </span>
                            @else
                                <span class="px-3 py-1 bg-red-100 text-red-700 rounded-full text-xs">
                                    {{ $row['status'] }}
                                </span>
                            @endif
                        </td>

                        <td class="p-3">
                            Rp{{ number_format($row->total_harga, 0, ',', '.') }}
                        </td>
                        <td class="p-3">
                                {{ $row->metode_pembayaran ?? '-' }}
                        </td>

                        <td class="p-3">
                            @if($row->bukti_pembayaran)
                                <img src="{{ asset($row->bukti_pembayaran) }}" class="w-20 h-20 object-cover">
                            @else
                                -
                            @endif
                        </td>

                        <td class="p-3">{{ $row->created_at->format('d-m-Y H:i') }}</td>

                        <td class="p-3">
                            @if($row->status == 'belum bayar')
                                <a href="{{ route('penyewa.booking.pembayaran', $row->booking_id) }}"
                                class="px-3 py-1 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">
                                    Bayar
                                </a>
                            @else
                                -
                            @endif
                        </td>
                    </tr>
                @endforeach

            </tbody>
        </table>

    </div>

</div>

@endsection
